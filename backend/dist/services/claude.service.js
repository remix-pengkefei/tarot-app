...
exports.claudeService = {
    async analyzeQuestion(question, availableSpreads) {
        try {
...
- ${spread.name}（${spread.cardCount}张牌）：${spread.description}
  适用场景：${JSON.parse(spread.positions).map((p) => p.meaning).join('、')}
`).join('\n')}
...
请返回以下格式的JSON响应：
{
  ...
  "recommendation": "<为什么推荐这个牌阵的原因>"
            ...
            if (content.type !== 'text') {
                throw new errorHandler_1.AppError(500, 'Unexpected response from Claude');
            }
...
给出一段温柔、有洞察力的解读（150-200字）。不要直接给出结论，而是帮助用户思考和理解。语言要通俗易懂，富有启发性。`;
    ...
    async generateSummary(question, cards) {
        try {
            if (cards.length <= 1) {
                return '';
            }
            const cardsDescription = cards.map(c => `- ${c.card.name}（${c.isReversed ? '逆位' : '正位'}）在"${c.position}"位置`).join('\n');
...
语气柔和有同理心，避免直接给出"好/坏"的结论，而是引导用户理解潜藏的含义与可能性。`;
    ...
    getSimpleSpreadRecommendation(question, spreads) {
        const questionLower = question.toLowerCase();
        let recommendedSpread;
        let analysis;
        let recommendation;
    ...
    getSimpleInterpretation(params) {
        const { question, card, position, isReversed } = params;
        const questionLower = question.toLowerCase();
    ...
    getSimpleSummary(question, cards) {
        if (cards.length <= 1)
            return '';
